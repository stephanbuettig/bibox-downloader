...
function checkDiskSpaceWindows(targetPath) {
    ...
    try {
        const output = (0, child_process_1.execSync)(`wmic logicaldisk where "DeviceID='${drive}'" get FreeSpace,Size /format:csv`, { encoding: 'utf-8', timeout: 5000, windowsHide: true });
...
function checkDiskSpaceUnix(targetPath) {
    // Sanitize path to prevent command injection
    const safePath = sanitizePath(targetPath);
    // Use df command with sanitized path
    const output = (0, child_process_1.execSync)(`df -B1 "${safePath}" 2>/dev/null`, {
        encoding: 'utf-8',
        timeout: 5000,
    });
    const lines = output.trim().split('\n');
...
//# sourceMappingURL=disk-check.js.map