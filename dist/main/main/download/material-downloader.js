...
exports.downloadMaterials = downloadMaterials;
...
const file_organizer_1 = require("../storage/file-organizer");
const mhtml_converter_1 = require("../storage/mhtml-converter");
 ...
 */
function detectMimeByMagicBytes(buf, bytesRead) {
    if (bytesRead < 4)
        return null;
    // MP3: ID3 tag header
...
async function downloadMaterials(materials, bookId, materialsDir, checkpoint, onProgress, abortSignal, throttle, // [Bug2-FIX] Accept throttle for pause support
matOptions) {
    // Ensure output directories — only create subdirs needed by the materials we'll download
    const neededSubdirs = new Set();
    for (const m of materials) {
        neededSubdirs.add((0, file_organizer_1.getTargetSubdir)(m.type || 'unknown'));
    }
    ...
    logger_1.logger.info('MatDL', `Starting: ${pendingMaterials.length} pending of ${materials.length} total materials`);
    let completed = checkpoint.getCompletedMaterialCount();
    const total = materials.length;
    let bytesDownloaded = 0;
    const errors = [];
    // Use adaptive concurrency from throttle (default 3, scales to 6)
    const concurrency = throttle ? Math.max(1, throttle.getConcurrency()) : types_1.BIBOX_CONSTANTS.MAX_PARALLEL_API;
    ...
    const concurrencyInterval = throttle ? setInterval(() => {
        ...
        if (newC !== queue.concurrency)
            queue.concurrency = newC;
    }, 2000) : null;
    // [Review2] Named factory function so failed tasks can be re-queued
    const createMaterialTask = (material) => async () => {
        ...
        try {
            ...
            const downloadUrl = await (0, bibox_api_1.fetchMaterialDownloadUrl)(matId, bookId);
            // Add small delay between API requests
            await (0, utils_1.sleep)(types_1.BIBOX_CONSTANTS.API_DELAY_MS);
            // Step 2+3: Pre-classify type (before download) for streaming decision
            const preType = (0, file_organizer_1.classifyMaterialType)(material.type, // Level 1: from sync data
            ...
            const result = await (0, client_1.downloadFileToPath)(downloadUrl, tmpPath, {
                ...
                try {
                    ...
                    if (!contentType.includes('xml')) {
                        ...
                        if (header.startsWith('<?xml') || header.startsWith('<xml')) {
                            ...
                            logger_1.logger.debug('MatDL', `Detected XML by content inspection: material ${matId}`);
                        }
                    }
                    ...
                    if (effectiveContentType === 'application/octet-stream' && bytesRead >= 4) {
                        ...
                        if (detected) {
                            ...
                            logger_1.logger.debug('MatDL', `Detected ${detected} by magic bytes: material ${matId}`);
                        }
                    }
                }
            ...
            const detectedType = (0, file_organizer_1.classifyMaterialType)(material.type, effectiveContentType, downloadUrl);
            // Skip XML metadata files — these are BiBox internal data, not real materials
            if (detectedType === 'xml') {
                logger_1.logger.info('MatDL', `Skipping XML metadata file: material ${matId} (${material.title || 'untitled'})`);
                try {
                    fs.unlinkSync(tmpPath);
                }
                catch { /* ignore */ }
            ...
            if (detectedType === 'html5' && !matOptions?.downloadHtml5) {
                logger_1.logger.info('MatDL', `Skipping HTML5 material (option disabled): material ${matId} (${material.title || 'untitled'})`);
                try {
                    fs.unlinkSync(tmpPath);
                }
                catch { /* ignore */ }
            ...
            if (detectedType === 'unknown') {
                logger_1.logger.info('MatDL', `Skipping unknown file type: material ${matId} (${material.title || 'untitled'}, ct=${contentType})`);
                try {
                    fs.unlinkSync(tmpPath);
                }
                catch { /* ignore */ }
            ...
            if (detectedType === 'html5' && contentType && (contentType.includes('text/html') || contentType.includes('application/xhtml'))) {
                try {
                    ...
                    const mhtmlContent = (0, mhtml_converter_1.htmlToMhtml)(htmlContent, downloadUrl, material.title || `Material_${matId}`);
                    fs.writeFileSync(tmpPath, mhtmlContent);
                    logger_1.logger.info('MatDL', `Converted HTML to MHTML: material ${matId}`);
                }
                catch (convErr) {
                    logger_1.logger.warn('MatDL', `MHTML conversion failed for material ${matId}, keeping as HTML`, convErr);
                }
            }
            ...
            const subdir = (0, file_organizer_1.getTargetSubdir)(detectedType);
            ...
            logger_1.logger.debug('MatDL', `Downloaded: ${filename} (${(size / 1024).toFixed(0)} KB, type=${detectedType}, ct=${contentType})`);
            completed++;
            ...
            logger_1.logger.warn('MatDL', `Failed material ${matId}: ${errorMsg}`);
            ...
            if (errorMsg.includes('401') || errorMsg.includes('403')) {
                ...
                if (retries <= 2) {
                    ...
                    await (0, utils_1.sleep)(types_1.BIBOX_CONSTANTS.RETRY_BASE_MS * Math.pow(2, retries));
...
function buildMaterialFilename(material, type, downloadUrl, contentType) {
    ...
    if (material.title) {
        parts.push((0, file_organizer_1.sanitizeFilename)(material.title));
    }
    else {
...
function getExtension(type, url, contentType) {
    // Priority 1: Content-Type header (most reliable after download)
    const ctExt = (0, file_organizer_1.contentTypeToExtension)(contentType);
    if (ctExt)
        return ctExt;
...
//# sourceMappingURL=material-downloader.js.map