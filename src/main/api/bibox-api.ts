...
const API = BIBOX_CONSTANTS.BACKEND_BASE;
const CDN = BIBOX_CONSTANTS.STATIC_CDN;

// --- Books ---

export async function fetchBooks(): Promise<Book[]> {
  const { data } = await apiRequest<unknown>(`${API}/api/books`);
  ...
  const booksArray = Array.isArray(data) ? data : (data as Record<string, unknown>).books || [];

  // [DEBUG] Log raw first book to understand API shape
  if (Array.isArray(booksArray) && booksArray.length > 0) {
    logger.debug('API', `fetchBooks raw sample: ${JSON.stringify(booksArray[0]).slice(0, 500)}`);
  }

  return (booksArray as Record<string, unknown>[]).map((b) => ({
...
export async function fetchIsbn(bookId: number): Promise<string | null> {
  try {
    const { data } = await apiRequest<unknown>(`${API}/api/books/${bookId}/getIsbn`);
...
export async function fetchPageData(bookId: number): Promise<{ pageCount?: number }> {
  try {
    const { data } = await apiRequest<Record<string, unknown>>(
      `${API}/api/books/${bookId}/pageData`
    );
    logger.debug('API', `pageData for book ${bookId}: ${JSON.stringify(data).slice(0, 300)}`);
    // Try multiple property names
    const count = extractNumber(data as Record<string, unknown>, ['pageCount', 'pages', 'totalPages', 'numPages', 'page_count', 'count']);
    // Also check if data itself is a number or the response is an array (count = length)
    if (count) return { pageCount: count };
    if (typeof data === 'number') return { pageCount: data as number };
    if (Array.isArray(data)) return { pageCount: data.length };
    return {};
  } catch {
    return {};
  }
...
export async function fetchSyncData(bookId: number): Promise<BookSyncData> {
  const { data } = await apiRequest<unknown>(`${API}/v1/api/sync/${bookId}`);
...
function extractPages(syncData: Record<string, unknown>, _bookId: number): PageInfo[] {
...
function extractMaterials(syncData: Record<string, unknown>, _bookId: number): MaterialInfo[] {
...
export async function fetchMaterialDownloadUrl(
  ...
  const { data } = await apiRequest<MaterialDownloadUrlResponse>(
    `${API}/api/materials/${materialId}/${bookId}/download-url?redirect=0`
  );
...
export async function fetchSaktMaterials(bookId: number): Promise<MaterialInfo[]> {
  try {
    const { data } = await apiRequest<unknown[]>(`${API}/api/sakt-material/${bookId}`);
...
function inferMaterialType(obj: Record<string, unknown>): 'pdf' | 'video' | 'audio' | 'html5' | 'image' | 'xml' | 'unknown' {
  const type = String(obj.type || obj.materialType || obj.contentType || '').toLowerCase();
  if (type.includes('pdf') || type === 'worksheet') return 'pdf';
  if (type.includes('video') || type === 'mp4') return 'video';
  if (type.includes('audio') || type === 'mp3') return 'audio';
  if (type.includes('html') || type.includes('interactive') || type === 'h5p') return 'html5';
  if (type.includes('image') || type === 'png' || type === 'jpg') return 'image';
...
function findCdnPathInObj(obj: Record<string, unknown>): string | null {
...
function findBase64Path(obj: unknown): string | null {
...
function findPageCount(obj: unknown): number {
...
function findAllStrings(obj: unknown, maxDepth = 10): string[] {
...
function findAllStringsMatching(obj: unknown, pattern: RegExp): string[] {
  return findAllStrings(obj).filter((s) => pattern.test(s));
}

function collectObjects(obj: unknown, maxDepth = 10): Record<string, unknown>[] {
  if (maxDepth <= 0) return [];
