...
export function classifyMaterialType(
  syncType: MaterialType | undefined,  // Level 1: from sync data
  contentType: string | undefined,      // Level 2: from Content-Type header (may be undefined)
  downloadUrl: string                   // Level 3: from URL extension
): MaterialType {
  ...
  if (ct === 'text/html' || ct === 'application/xhtml+xml') return 'html5';
  if (ct === 'application/zip' || ct === 'application/x-zip-compressed') return 'html5';
  if (ct === 'application/xml' || ct === 'text/xml') return 'xml';
  if (ct === 'application/octet-stream') {
    // Generic binary — fall through to URL extension detection
  }
  if (ct === 'application/vnd.openxmlformats-officedocument.wordprocessingml.document') return 'pdf'; // docx → Arbeitsblätter
  if (ct === 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet') return 'pdf'; // xlsx → Arbeitsblätter
  if (ct === 'application/vnd.openxmlformats-officedocument.presentationml.presentation') return 'pdf'; // pptx → Arbeitsblätter
  if (ct === 'application/msword' || ct === 'application/vnd.ms-excel' || ct === 'application/vnd.ms-powerpoint') return 'pdf'; // Office → Arbeitsblätter
...
export function getTargetSubdir(type: MaterialType): string {
  switch (type) {
