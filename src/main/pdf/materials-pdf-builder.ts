...
import { isWordAvailable, convertAllDocsViaWord } from './word-to-pdf-converter';
...
export async function buildMaterialsPdf(
  bookDir: string
  ...
  try {
    ...
    if (manifest.title) bookTitle = manifest.title;
  } catch { /* use folder name */ }

  logger.info('MatPDF', `Starting materials PDF build for "${bookTitle}"`);
  ...
  logger.info('MatPDF', `Found ${docFiles.length} convertible files (${wordFiles.length} DOC/DOCX), ${skippedFiles.length} skipped`);
  ...
  try {
    ...
    for (const file of docFiles) {
      try {
        ...
        logger.warn('MatPDF', `Failed to process "${file.filename}": ${msg}`);
        addErrorPage(mergedPdf, font, file.filename, msg);
        failedCount++;
      }
    }

    logger.info('MatPDF', `Processed ${processedCount} files, ${failedCount} failed`);
    ...
    logger.info('MatPDF', `Materials PDF saved: "${outputFilename}" (${sizeMB} MB, ${mergedPdf.getPageCount()} pages)`);

    cleanupTempDir(bookDir);
    return { success: true, path: outputPath };
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    logger.error('MatPDF', `Materials PDF build failed: ${msg}`, err);
    cleanupTempDir(bookDir);
 ...
 */
async function preConvertWordFiles(
  wordFiles: MaterialFile[],
  bookDir: string,
): Promise<Map<string, string>> {
  if (wordFiles.length === 0) return new Map();
  ...
  const wordAvail = await isWordAvailable();

  if (wordAvail) {
    logger.info('MatPDF', `MS Word available — converting ${wordFiles.length} Word files via COM`);

    const wordResults = await convertAllDocsViaWord(
      wordFiles.map((f) => f.filePath),
      path.join(tmpDir, 'word'),
      {
        timeoutMs: 600_000,
        onProgress: (done, total) => {
          logger.debug('MatPDF', `Word COM progress: ${done}/${total}`);
        },
        onStatus: (msg) => {
          logger.info('MatPDF', `Word: ${msg}`);
        },
      },
    );
    ...
    logger.info('MatPDF', `Word COM: ${resultMap.size}/${wordFiles.length} converted in ${elapsedSec}s, ${remainingFiles.length} remaining`);
  } else {
    logger.warn('MatPDF', 'MS Word NOT available — falling back to text extraction');
  }
  ...
  if (remainingFiles.length > 0) {
    ...
    for (const file of remainingFiles) {
      try {
        ...
        logger.debug('MatPDF', `Text extraction failed for "${file.filename}": ${err instanceof Error ? err.message : String(err)}`);
      }
    }
    logger.info('MatPDF', `Text extraction: ${textOk} more converted`);
  }

  const totalElapsed = ((Date.now() - startTime) / 1000).toFixed(1);
  logger.info('MatPDF', `All Word conversions done: ${resultMap.size}/${wordFiles.length} in ${totalElapsed}s`);
 ...
 */
async function convertViaTextExtraction(
  filePath: string,
  outputDir: string,
): Promise<string | null> {
...
function collectDocumentFiles(materialsDir: string): MaterialFile[] {
  ...
  const pdfBaseNames = new Set<string>(); // "dir/basename" or just "basename"

  const allEntries = fs.readdirSync(materialsDir, { withFileTypes: true });

  // --- Root-level files ---
  const rootFiles: Array<{ name: string; filePath: string }> = [];
  ...
  // --- Subdirectory files ---
  const subFiles: Array<{ name: string; filePath: string; dirName: string }> = [];
  for (const entry of allEntries) {
  ...
  if (deduped > 0) {
    logger.info('MatPDF', `Deduplicated: ${deduped} DOC/DOCX files skipped (PDF version available)`);
  }
...
function classifyFileType(ext: string): MaterialFile['type'] {
  switch (ext) {
    case '.pdf': return 'pdf';
...
function extractSortKey(filename: string): string {
  ...
  if (match) return match[1].padStart(4, '0') + '_' + filename;
...
function createCoverPage(
  ...
  font: PDFFont,
  bookTitle: string,
  docFiles: MaterialFile[],
  skippedFiles: MaterialFile[],
): void {
  const page = doc.addPage([A4_WIDTH, A4_HEIGHT]);
  let y = A4_HEIGHT - MARGIN;

  page.drawText('Materialien-Sammlung', {
    x: MARGIN, y, size: 22, font: fontBold, color: rgb(0.05, 0.09, 0.16),
  });
  y -= 30;

  page.drawText(sanitizeWinAnsi(bookTitle), {
    x: MARGIN, y, size: 14, font: font, color: rgb(0.23, 0.51, 0.96),
  });
  y -= 24;
...
function addSeparatorPage(
  ...
  page.drawText(`Dokument ${index} / ${total}`, {
    x: MARGIN, y: startY + 30, size: 11, font, color: rgb(0.5, 0.5, 0.5),
  });
  ...
  for (const line of wrappedLines) {
    page.drawText(line, {
      x: MARGIN, y, size: 14, font: fontBold, color: rgb(0.05, 0.09, 0.16),
    });
    y -= 20;
  }
...
function wrapFilename(text: string, font: PDFFont, size: number, maxWidth: number): string[] {
  if (!text) return [''];
...
function addErrorPage(doc: PDFDocument, font: PDFFont, filename: string, errorMsg: string): void {
  const page = doc.addPage([A4_WIDTH, A4_HEIGHT]);
  const y = A4_HEIGHT / 2;

  page.drawText('Konvertierung fehlgeschlagen', {
    x: MARGIN, y: y + 20, size: 14, font, color: rgb(0.8, 0.2, 0.2),
  });
  page.drawText(sanitizeWinAnsi(filename), {
    x: MARGIN, y, size: 11, font, color: rgb(0.3, 0.3, 0.3),
  });

  const shortError = errorMsg.length > 120 ? errorMsg.slice(0, 117) + '...' : errorMsg;
  page.drawText(sanitizeWinAnsi(shortError), {
    x: MARGIN, y: y - 20, size: 9, font, color: rgb(0.5, 0.5, 0.5),
  });
}
 ...
 */
async function decryptPdfViaChromium(
  inputPath: string,
  outputDir: string,
): Promise<string | null> {
  ...
  try {
    ...
    logger.debug('MatPDF', `Loading encrypted PDF in Chromium: ${fileUri}`);
    ...
    if (stat.size < 5000) {
      logger.warn('MatPDF', `Chromium decrypt produced tiny file (${stat.size} bytes) — likely empty`);
      try { fs.unlinkSync(outputPath); } catch { /* ignore */ }
      return null;
    }

    logger.info('MatPDF', `PDF decrypted via Chromium: "${baseName}.pdf" (${(stat.size / 1024).toFixed(1)} KB)`);
    return outputPath;
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    logger.warn('MatPDF', `Chromium PDF decrypt failed for "${baseName}.pdf": ${msg}`);
    return null;
  } finally {
 ...
 */
function isPdfEncrypted(fileBytes: Buffer): boolean {
...
async function mergePdfFile(
  ...
  if (encrypted) {
    logger.info('MatPDF', `Verschluesseltes PDF erkannt: "${path.basename(filePath)}" — Entschluesselung via Chromium`);
    ...
    if (decryptDir) {
      ...
      if (decryptedPath) {
        ...
        try {
          const srcDoc = await PDFDocument.load(decryptedBytes);
          const pages = await targetDoc.copyPages(srcDoc, srcDoc.getPageIndices());
          for (const page of pages) {
            targetDoc.addPage(page);
          }
          logger.info('MatPDF', `Verschluesseltes PDF entschluesselt und eingefuegt: "${path.basename(filePath)}" (${pages.length} Seiten)`);
          return;
        } catch (err) {
          logger.warn('MatPDF', `Entschluesseltes PDF nicht lesbar: "${path.basename(filePath)}" — ${err instanceof Error ? err.message : String(err)}`);
        }
      }
    }
 ...
 */
async function extractTextFromOLE2Doc(filePath: string): Promise<string> {
...
function extractTextFromDocxZip(filePath: string): string {
  const buf = fs.readFileSync(filePath);
...
function renderTextToPages(
  ...
  const drawPageHeader = () => {
    page.drawText(sanitizeWinAnsi(sourceFilename), {
      x: MARGIN, y: A4_HEIGHT - 30, size: 7, font, color: rgb(0.6, 0.6, 0.6),
    });
    page.drawLine({
    ...
    for (const line of wrappedLines) {
      ...
      if (line.trim().length > 0) {
        page.drawText(sanitizeWinAnsi(line), {
          x: MARGIN, y, size: BODY_SIZE, font, color: rgb(0.1, 0.1, 0.1),
        });
      }
      y -= LINE_HEIGHT;
    }
...
async function embedImageAsPage(doc: PDFDocument, filePath: string): Promise<void> {
...
function cleanupTempDir(bookDir: string): void {
  const tmpDir = path.join(bookDir, '.tmp-pdf-convert');
  try {
    if (fs.existsSync(tmpDir)) {
      fs.rmSync(tmpDir, { recursive: true, force: true });
      logger.debug('MatPDF', 'Cleaned up temp conversion directory');
    }
  } catch (err) {
    logger.debug('MatPDF', `Failed to cleanup temp dir: ${err instanceof Error ? err.message : String(err)}`);
  }
}
