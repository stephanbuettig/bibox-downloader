...
interface PngInfo {
  ...
  bitDepth: number;
  colorType: number;
  /** For colorType 3 (indexed): raw PLTE chunk data (N×3 bytes RGB) */
  palette?: Buffer;
  /** For colorType 3 with tRNS: transparency data */
  transparency?: Buffer;
}

function readPngInfo(pngBytes: Buffer): PngInfo {
  ...
  if (info.colorType === 3) {
    ...
    while (pos + 12 <= pngBytes.length) {
      ...
      if (pos + 12 + chunkLen > pngBytes.length) break;
...
function extractIdatChunks(pngBytes: Buffer): Buffer {
  const chunks: Buffer[] = [];
  let pos = 8; // skip PNG signature
  while (pos + 12 <= pngBytes.length) {
    const chunkLen = pngBytes.readUInt32BE(pos);
    ...
    if (chunkType === 'IEND') break; // No more chunks after IEND
    pos += 12 + chunkLen;
  }
  if (chunks.length === 0) {
...
export async function buildPdf(
  ...
  logger.info('PDF', `[v3] Building PDF from ${files.length} PNGs: "${metadata.title}" (streaming mode)`);
  ...
  logger.info('PDF', `Reference page size: ${refWidth}×${refHeight} (from ${files[0]})`);

  // --- Phase 2: Build PDF directly to file ---
  const tmpPath = outputPath + '.tmp';
  const fd = fs.openSync(tmpPath, 'w');
  let offset = 0;
  const objOffsets: number[] = []; // byte offset of each object (for xref)
  ...
  const writeBuffer = (buf: Buffer): void => {
    fs.writeSync(fd, buf);
    offset += buf.length;
  };

  const startObj = (objNum: number): void => {
  ...
  if (metadata.author) write(' /Author (' + escapePdf(metadata.author) + ')');
  if (metadata.isbn) write(' /Subject (ISBN: ' + escapePdf(metadata.isbn) + ')');
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    try {
      ...
      logger.warn('PDF', `Failed to read ${filename}: ${err instanceof Error ? err.message : String(err)}`);
      continue;
    }
    ...
    try {
      ...
      logger.warn('PDF', `Invalid PNG ${filename}: ${err instanceof Error ? err.message : String(err)}`);
      continue;
    }
    ...
    try {
      ...
      logger.warn('PDF', `No IDAT in ${filename}: ${err instanceof Error ? err.message : String(err)}`);
      continue;
    }
    ...
    if (pngInfo.colorType === 0) {
      ...
      logger.warn('PDF', `Unknown PNG colorType ${pngInfo.colorType} in ${filename}, treating as grayscale`);
      colorSpace = '/DeviceGray';
      colors = 1;
    }
  ...
  logger.info('PDF', `[v3] PDF saved: ${sizeMB} MB, ${pagesWritten} pages, ${elapsed}s (streaming mode)`);
}
